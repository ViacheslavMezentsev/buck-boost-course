...
void StateMRise(void)
{
	...
	switch(STState)
	{
		...
		case    SSRun:
		{
			if(DF.PWMENFlag==0) // Очистка переменных до начала
			{
				...
				IErr0 = 0;
				i0 = 0;
				HAL_HRTIM_WaveformOutputStart(&hhrtim1, HRTIM_OUTPUT_TA1|HRTIM_OUTPUT_TA2); // Включение выхода и таймера
				HAL_HRTIM_WaveformOutputStart(&hhrtim1, HRTIM_OUTPUT_TB1|HRTIM_OUTPUT_TB2); // Включение выхода и таймера
			}
			// Устанавливаем флаг
			DF.PWMENFlag=1;
...
void ValInit(void)
{
	...
	CtrValue.Voref=0;
	// Опорное значение ограничения тока
	CtrValue.ILimit  = I_LIMIT;
...
void MX_OLED_Init(void)
{
  ...
  ST7789_SetBL(100);
	ST7789_print_7x11(10,0,GREEN,BLACK,1,"Buck VLoop PI ILimit");
...
**     Description : Отображение основных параметров на экране
**     Parameters  :
**     Returns     :
** ===================================================================*/
void OLEDShow(void)
