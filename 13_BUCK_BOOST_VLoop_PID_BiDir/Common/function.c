...
CCMRAM void ADCSample(void)
{
	...
	SADC.Iin  = ((SADC.IinOffset - ADC1_RESULT[3])*CAL_IIN_K>>12)+CAL_IIN_B;
	SADC.Vout  = ((uint32_t )ADC1_RESULT[0]*CAL_VOUT_K>>12)+CAL_VOUT_B;
	SADC.Iout  = ((SADC.IoutOffset - ADC1_RESULT[1] )*CAL_IOUT_K>>12)+CAL_IOUT_B;
...
void StateMRise(void)
{
	...
	switch(STState)
	{
		case    SSInit:
		{
			// Отключаем PWM
			DF.PWMENFlag=0;
			HAL_HRTIM_WaveformOutputStop(&hhrtim1, HRTIM_OUTPUT_TA1|HRTIM_OUTPUT_TA2); //№Ш±Х
			HAL_HRTIM_WaveformOutputStop(&hhrtim1, HRTIM_OUTPUT_TB1|HRTIM_OUTPUT_TB2); //№Ш±Х			
		...
		case    SSWait:
		{
			...
			if(Cnt> MAX_SSCNT)
			{
				...
				u0 = 0;
				u1 = 0;
				//CtrValue.Voref Выходное опорное напряжение уменьшается вдвое чтобы не допустить перерегулирования
		...
		case    SSRun:
		{
			if(DF.PWMENFlag==0) // Очистка переменных перед началом работы
			{
				// Инициализация переменных
				VErr0=0;
				VErr1=0;
				...
				u0 = 0;
				u1 = 0;
				HAL_HRTIM_WaveformOutputStart(&hhrtim1, HRTIM_OUTPUT_TA1|HRTIM_OUTPUT_TA2); // Запуск PWM
				HAL_HRTIM_WaveformOutputStart(&hhrtim1, HRTIM_OUTPUT_TB1|HRTIM_OUTPUT_TB2); // Запуск PWM		
			}
			// Устанавливаем флаг
			DF.PWMENFlag=1;
...
void ValInit(void)
{
	...
	DF.ErrFlag=0;
	// Инициализация опорного напряжения
	CtrValue.Voref=0;
...
void VrefGet(void)
{
	...
	// Предел регулирования в режиме MIX
	if(CtrValue.Voref >(SADC.VinAvg<<1))  
...
**     Description : Выбор режимов работы:
**                   Режим BUCK: выходное опорное напряжение < 0,8 входного напряжения.
**                   Режим BOOST: выходное опорное напряжение > 1,2 входного напряжения.
**                   Режим MIX: входное напряжение в 1,15 раза > выходное опорное напряжение > входное напряжение в 0,85 раза
**                   При входе в режим MIX его необходимо замедлить чтобы предотвратить колебания в критической точке.
**     Parameters  :
**     Returns     :
** ===================================================================*/
void BBMode(void)
{
	...
	// Временно сохраняем текущий режим
	PreBBFlag = DF.BBFlag;
	
	// Определяем текущий режим работы
	switch(DF.BBFlag)
	{
		// NA – режим инициализации
		case  NA :		
		{
	...
	// Если режим изменился, то устанавливаем флаг смены режима чтобы предотвратить выбросы и возможное перерегулирование
...
void MX_OLED_Init(void)
{
  ...
  ST7789_SetBL(100);
	ST7789_print_7x11(10,0,GREEN,BLACK,1,"V-Loop PID BiDir");
	ST7789_print_7x11(10,20,GREEN,BLACK,1,"State:");
	ST7789_print_7x11(60,60,GREEN,BLACK,1,"LEFT     RIGHT");
...
**     Description : Вывод значений и режимов на экран
**     Parameters  :
**     Returns     :
** ===================================================================*/
void OLEDShow(void)
{
	...
	if(BBFlagTemp!= DF.BBFlag)
	{
		...
		switch(DF.BBFlag)
		{
			case  NA :		
			{
				ST7789_print_7x11(60,20,GREEN,BLACK,0,"MODE:*NA* ");
				break;
			}
			case  Buck :		
			{
				ST7789_print_7x11(60,20,GREEN,BLACK,0,"MODE:BUCK ");
				break;
			}
			case  Boost :		
			{
				ST7789_print_7x11(60,20,GREEN,BLACK,0,"MODE:BOOST");
				break;
			}
			case  Mix :		
			{
				ST7789_print_7x11(60,20,GREEN,BLACK,0,"MODE:MIX  ");
				break;
			}
		}
	}
