...
**     Description :    Функция получения значений напряжения и токов
**     Parameters  :
**     Returns     :
** ===================================================================
*/
...
void SwOCP(void)
{
	...
	// Когда выходной ток больше ограничения дольше чем 50ms
...
/**
 * @brief 	Инициализируем экран и основные надписи.
 * 
 */
void MX_OLED_Init(void)
{
	ST7789_print_7x11(10,0,GREEN,BLACK,1,"BOARD TEST");
...
/**
 * @brief 	Динамическое обновление значений.
 * 
 */
void OLEDShow(void)
{
    ...
    {
        { VinT, 'V', 40, 80 },  { VoutT, 'V', 130, 80 },
        { IinT, 'A', 40, 100 }, { IoutT, 'A', 130, 100 },
        { ( VoutT * IoutT ) / 100, 'W', 40, 120 },
        { ( VinT * IinT ) / 100, 'W', 130, 120 },
        { VadjT, 'V', 60, 150 },
    };

    char text[16];

    uint8_t count = sizeof( items ) / sizeof( items[0] );

    for ( uint8_t n = 0; n < count; n++ )
    {
        sprintf( text, "%2lu.%02lu %c ", items[n].value / 100, items[n].value % 100, items[n].unit );
        ST7789_print_7x11( items[n].x, items[n].y, GREEN, BLACK, 0, text );
    }
#endif
