	...
	switch(STState)
	{
		...
		case    SSWait:
		{
			...
			if(Cnt> MAX_SSCNT)
			{
				...
				IErr0=0;
				i0 = 0;
				// CtrValue.Ioref Начинаем с половинного значениея чтобы не допустить перерегулирования
				CtrValue.Ioref  = CtrValue.Ioref >>2;
		...
		case    SSRun:
		{
	        ...
	        if(DF.PWMENFlag==0)
			{
			  // Инициализируем переменные
				IErr0=0;
				i0 = 0;
				HAL_HRTIM_WaveformOutputStart(&hhrtim1, HRTIM_OUTPUT_TA1|HRTIM_OUTPUT_TA2); // Включаем PWM выход и PWM таймер
				HAL_HRTIM_WaveformOutputStart(&hhrtim1, HRTIM_OUTPUT_TB1|HRTIM_OUTPUT_TB2); // Включаем PWM выход и PWM таймер
			}
			// Устанавливаем флаг
			DF.PWMENFlag=1;
...
**     Description  :  Получение опорного значения тока
**     Parameters   :
**     Returns      :
** ===================================================================*/
#define MAX_IREF    931  // Максимальный ток 5А
#define MIN_IREF    0    // Минимальный ток 0A
#define IREF_K      10   // Шаг регулирования

void IrefGet(void)
{
	// Промежуточная переменная опорного значения тока
	int32_t ITemp = 0;	
	// Скользящее среднее
...
**                   BUCK mode: Output reference voltage <0.8 times input voltage
**                   BOOST mode: Output reference voltage >1.2 times input voltage
**                   MIX mode: 1.15 times input voltage > output reference voltage > 0.85 times input voltage
**                   When entering MIX (buck-boost) mode, it needs to be slowed down when exiting to BUCK or BOOST to prevent oscillation back and forth at the critical point.
**     Parameters  :
**     Returns     :
** ===================================================================*/
void BBMode(void)
...
void MX_OLED_Init(void)
{
  ...
  ST7789_SetBL(100);
	ST7789_print_7x11(10,0,GREEN,BLACK,1,"Buck-Boost I-Loop PI");
	...
	ST7789_print_7x11(10,150,GREEN,BLACK,1,"Iadj:");
}

/** ===================================================================
**     Funtion Name :void OLEDShow(void)
**     Description : Подготовка и отображение параметров на экране
**     Parameters  :
**     Returns     :
** ===================================================================*/
void OLEDShow(void)
{
	...
	sprintf(dd, "%0*.*f A ",4,2,(float)IadjT/100);
